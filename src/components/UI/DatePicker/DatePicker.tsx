  ...
  return (
    <div className="rounded-xxl">
      <input
        ...
        className="w-32 h-8 bg-ui_surface_opc rounded-lg px-2 text-body_light_dark text-x tracking-wider font-medium text-left border-none outline-none cursor-pointer"
        value={props.selectedDateTimestamp.formatted}
        onChange={(e) => {
          if (Date.parse(e.target.value) < Date.parse(new Date().toString()))
            return false;
